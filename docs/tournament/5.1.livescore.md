...
If you select your matches from a feed, this will typically happen automatically.

For manual matches, you can simply do this in the settings screen by choosing 'Live Score / Chrome Cast' and than 'Support live scoring'.
(Or check the `Update live score` checkbox when you enter a match).

After choosing this option:
* the 'Share' feature of Squore is set to 'Automatic', and
* the score sheet of the match is shared (updated) to the server after each game or even each point (configurable)
...
- by one or more player names
- by one or more player countries
