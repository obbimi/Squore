...
- start a new match on a 'remote' device 
...
Once the dialog is opened on both devices,
- select what role each device should play. One is typically 'master' and one is 'slave'
- The device on which you choose 'slave' presents you with a list of devices under 'Select other device id'. 
  Here you should select the 'other' device that you will make 'master'

Start entering the score on the 'master' device. The 'slave' should follow suite.
Beware of the option `Settings > Appearance > Layout > Landscape Layout`. 
Here you can select a 'presentation' layout that is ... well more for 'presenting' the score and less for 'changing' the score.
...
- run an MQTT broker locally (see below)
  - (although for testing I was able to achieve the same using the public mqtt server `test.mosquitto.org` )
...
If you don't want to use MQTT anymore you can 
- turn it of in the settings:
  - Settings > LiveScore/ChromeCast/MQTT/Bluetooth Mirror > MQTT
- Long-click on the little `MQ:#` icon 'within' the score button of player A 
